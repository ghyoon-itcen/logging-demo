...
- 경고만 보기: `|= "WARN"`
- 자동 생성 로그만 보기: `|= "[AUTO]"`
- 특정 요청 추적 (reqId): `|= "a3f8b2c1"`
- 특정 API만 보기: `|= "/demo/logs/error"`
...
`spring-petclinic-main/src/main/resources/logback-spring.xml`에서 설정합니다:

```xml
<!-- 특정 패키지 로그 레벨 조정 -->
<logger name="org.springframework" level="INFO" />

<!-- 데모 컨트롤러/스케줄러 DEBUG 로그 보기 -->
<logger name="org.springframework.samples.petclinic.system" level="DEBUG" />
...
> 주의: `<root level="DEBUG">`로 전체 설정하면 Spring 내부 로그가 대량 출력됩니다. `<logger>` 태그로 패키지 단위 지정을 권장합니다.

#### 로그 출력 형식 변경

`logback-spring.xml`의 `LOG_PATTERN` 프로퍼티를 수정합니다:

```xml
<property name="LOG_PATTERN" value="%d{yyyy-MM-dd HH:mm:ss.SSS} [%thread] %-5level %logger{36} - [%X{reqId:-}] [%X{method:-} %X{uri:-}] %msg%n" />
```
...
| 필드 | 설명 | 예시 |
|---|---|---|
| `%X{reqId}` | 요청별 고유 ID (8자리) | `a3f8b2c1` |
| `%X{method}` | HTTP 메서드 또는 `SCHEDULER` | `GET`, `POST`, `SCHEDULER` |
| `%X{uri}` | 요청 경로 | `/demo/logs/info`, `/auto-log` |

출력 예시:
```
2025-07-07 10:23:45.123 [http-nio-8080-exec-3] INFO  LogDemoController - [a3f8b2c1] [GET /demo/logs/info] 사용자 요청 처리 완료 - 정상 동작
2025-07-07 10:23:50.001 [scheduling-1] INFO  LogDemoScheduler - [7e2d9f04] [SCHEDULER /auto-log] [AUTO] 스케줄러 정상 실행 - 다음 실행까지 5초
```

#### 로그 파일 롤링 정책

일별로 로그 파일이 롤링되며, 최대 30일간 보관됩니다 (최대 1GB):
...
- Logback 설정: `spring-petclinic-main/src/main/resources/logback-spring.xml`
- MDC 필터: `spring-petclinic-main/src/main/java/org/springframework/samples/petclinic/system/MdcFilter.java`
